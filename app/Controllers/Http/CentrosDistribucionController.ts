...
export default class CentrosDistribucionController {
    public async find({ request, params }: HttpContextContract) {
        if (params.id) {
            let theCentrosDistribucion: CentrosDistribucion = await CentrosDistribucion.findOrFail(params.id)
    ...
    public async create({ request }: HttpContextContract) {
        ...
        const theCentrosDistribucion: CentrosDistribucion = await CentrosDistribucion.create(body);
        //await theCentrosDistribucion.load("direccion")
        return theCentrosDistribucion;
    }

    public async update({ params, request }: HttpContextContract) {
        const theCentrosDistribucion: CentrosDistribucion = await CentrosDistribucion.findOrFail(params.id);
        const body = request.body();
        theCentrosDistribucion.nombre = body.nombre;
        theCentrosDistribucion.telefono = body.telefono;   
        theCentrosDistribucion.direccion_id = body.theater_id;   
